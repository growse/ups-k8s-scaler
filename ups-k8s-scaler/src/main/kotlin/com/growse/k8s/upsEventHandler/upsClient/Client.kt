...
/**
 * A client for a remote NUT instance that knows how to list available UPS devices, and then monitor
 * their status
 *
...
class Client(
  ...
  /**
   * Converts a list of lines received from the transport into a [UPSResponse]. A single response
   * can be spread across multiple lines, or not, as the case may be.
   *
    ...
    return if (responseLines.isEmpty()) {
      ...
      when (responseLines.first().substring(beginPrefix.length)) {
        "LIST UPS" -> {
          UPSResponse.UPSList(
              responseLines
                  .subList(1, responseLines.size - 1)
          ...
          when (val upsState = UPSStates.parse(upsStateValue.value)) {
            ...
            UPSStates.LowBattery -> {
              if (previousState == null || previousState != upsState) {
                ...
                job = CoroutineScope(Dispatchers.Default).launch { callbackMap[upsState]?.invoke() }
              }
            }
            else -> {
  ...
  /**
   * Connects the underlying transport, grabs the list of UPS's from the remote instance and starts
  ...
  private fun getUPSStatus(name: String) =
      sendCommand("GET VAR ${name.filter { !it.isWhitespace() }} ups.status")
  ...
  enum class UPSStates(val statusCode: String) {
    ...
    companion object {
       ...
       */
      fun parse(arg: String): UPSStates? =
          values().firstOrNull { it.statusCode == arg.split(" ")[0] }
    }
  }
