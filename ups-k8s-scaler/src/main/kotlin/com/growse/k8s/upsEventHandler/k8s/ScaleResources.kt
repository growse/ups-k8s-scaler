...
fun checkK8sConnectivity(): Result<Unit> = StorageV1Api().listStorageClass(labelSelector = STORAGE_CLASS_LABEL_SELECTOR).map {}
...
suspend fun scaleK8sResources(
    ...
    val storageClassNames =
        StorageV1Api().listStorageClass(labelSelector = STORAGE_CLASS_LABEL_SELECTOR).getOrElse {
            logger.error(it) { "Unable to list StorageClassNames" }
            return
        }.items.mapNotNull { storageClass -> storageClass.metadata?.name }

    yield()

    // all persistent volumumes using those storage classes
    val persistentVolumesWithStorageClassNames =
        CoreV1Api().listPersistentVolumeClaimForAllNamespaces().getOrElse {
            logger.error(it) { "Unable to list PersistentVolumeClaims" }
            return
        }.items.filter { storageClassNames.contains(it.spec?.storageClassName) }.mapNotNull { it.metadata?.name }

    yield()

    // all statefulsets with volume claims using those storage classes
    val statefulSetsWithVolumesUsingStorageClasses =
        AppsV1Api().listStatefulSetForAllNamespaces().getOrElse {
    ...
    val deploymentsWithVolumesUsingStorageClasses =
        AppsV1Api().listDeploymentForAllNamespaces().getOrElse {
            logger.error(it) { "Unable to list Deployments" }
            return
        }.items.filter { deployment ->
            deployment.metadata?.labels?.containsKey(ORDER_LABEL_KEY) ?: false || deployment.spec?.template?.spec?.volumes?.any {
    ...
    (statefulSetsWithVolumesUsingStorageClasses + deploymentsWithVolumesUsingStorageClasses).sortedBy {
        // If we're going online, we need to do everything in reverse order
        when (scaleDirection) {
            ScaleDirection.UP -> -1
            ScaleDirection.DOWN -> 1
        } * (it.whichever().metadata.labels?.getOrDefault(ORDER_LABEL_KEY, null)?.toInt() ?: DEFAULT_ORDER_VALUE)
    }.map {
        val desiredReplicas =
            when (scaleDirection) {
                ScaleDirection.UP -> 1
